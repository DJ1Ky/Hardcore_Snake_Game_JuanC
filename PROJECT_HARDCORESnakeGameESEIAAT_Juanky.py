...
# Joc de l'Snake fet a Python, per princiants, amb obstàcles (HARDCORE MODE) 
...
obstacle = turtle.Turtle()           # creem un objecte turtle perque es mostri algo a la pantalla 
obstacle.speed(0)                    # per a que s'inicii la pantalla, el cap de la serp estigui des de l'inici
...
obstacle.goto(0,150)                 # començará a la posició (0,0) de la pantalla 
obstacle.direction = "stop"          # amb aixó direccionem el primer obstacle a la posició desitjada. 
...
#game_over.write(f" GAME OVER " , align = "center" , font = ("Impact", 55, "bold"))      #  que en aquest cas es una tupla (tipus, tamany, negreta)
...
finestra.onkeypress(abaix, "Down")      # La primera lletra de la vocal del segon parámetre, ha de ser majúscula per fer referéncia a una tecla del teclat.
...
while True:                     # creem un bucle principal, ja que será uhn bucle infinit que fins que no li donem ordre de que surti del joc, mai acabará.
    ...
    if cap.distance(food) < 20:              # si la distanca es menor al tamany dels dos objectes, significará que s'han tocat
       ...
       # L'obstacle canvia de posició a una posició random cada cop que la serp aconsegueix menjar

        x = random.randint(-290, 290)           # creem números random per assignarlos a la nostra X i Y 
        y = random.randint(-290, 290)           # creem un núm. enter amb possibilitat de que surti dels -290 fins als 290 per a x i y, per no estar tan aprop del marge de la finestra
        obstacle.goto(x,y)                      # per actualitzar la possició de l'obstacle 
    
        nou_obstacle = turtle.Turtle()          # creem un objecte turtle perque es mostri algo a la pantalla 
        nou_obstacle.speed(0)                   # per a que s'inicii la pantalla, el nou segment del nou obstacle no es mogui
        nou_obstacle.shape("square")            # la forma del cos també serà quadrada
        nou_obstacle.color("black")             # li donem un color al obstacle
        nou_obstacle.penup()                    # amb aquest comand, no hi haurá rastre o estela
        nou_obstacle.goto(x,y)                  # un nou segment apareix en una posició aleatoria dins el marc de la finetra de joc
        obstacles.append(nou_obstacle)          # cada cop que es crei l'obstacle, s'anirà afegint a la llista "obstacles"
    ...
    for obstacle in obstacles:                              # bucle for per iterar la llista d'obstacles
        if cap.distance(obstacle) < 25:                     # si la distanca es menor al tamany dels dos objectes (cap i obstacle), significará que s'han tocat
            time.sleep(1)                                   # s'adorm el programa un segon
            cap.goto(0,0)                                   # reset al programa fent qque el cap torni a la posició d'origen (0,0)
            cap.direction = "stop"                          # i la direcció estigui en mode "stop"
            ...
            for segment in segments_cos:                    # Per cada segment que estigui a la llista "segments_cos"
                segment.goto(1111,1111)                     # els fem anar ben lluny (fora dels marcs de la finiestra (1111,1111))
                obstacles.clear()                           # Borrem els obstacles que queden (no funciona i no s'envan)
            segments_cos.clear()                            # Borrem els segments de la llista, però fora del bucle for, ja que sinó els segments quedarien dins de la finestra  
   ...
   # paràmetre será que vagi decreixent el valor (-1).
    ...
    for segment in segments_cos:
        if segment.distance(cap) < 20:      # Fem que la distancia amb el segments del cos no estigui tan aprop del cap de la serp
            ...
            segments_cos.clear()                # Borrem els segments de la llista, però fora del bucle for, ja que sinó els segments quedarien dins de la finestra  
            obstacle.clear()                    # Intento borrar els obstacles de la llista, però no puc fe-los desaparèixer
            obstacle = 0
            # Reset del resultat del marcador
            resultat = 0
